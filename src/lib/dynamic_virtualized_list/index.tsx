...
import type {
    DynamicVirtualizedListProps,
    DynamicVirtualizedListState,
    ItemMetadata,
    ListItemKey,
    ListItemStyle,
    ListMeasurementProps,
    ListMetaData,
    ScrollAlign,
    ScrollSnapshot,
} from './types';
...
export type {
    DynamicVirtualizedListChildrenProps,
    DynamicVirtualizedListProps,
    InitScrollToIndexResult,
    OnItemsRenderedArgs,
    OnScrollArgs,
    ScrollAlign,
    ScrollDirection,
...
> {
    ...
    _itemStyleCache: Record<ListItemKey, ListItemStyle> = {};
    ...
    static defaultProps: Partial<DynamicVirtualizedListProps> = {
        innerTagName: 'div',
        itemData: undefined,
        outerTagName: 'div',
    ...
    state: DynamicVirtualizedListState = {
        scrollDirection: 'backward',
        scrollOffset: typeof this.props.initialScrollOffset === 'number' ? this.props.initialScrollOffset : 0,
        scrollUpdateWasRequested: false,
        scrollDelta: 0,
        scrollHeight: 0,
        localOlderPostsToRender: [0, 0],
    };

    // Always use explicit constructor for React components.
    // It produces less code after transpilation. (#26)
    // eslint-disable-next-line no-useless-constructor
    constructor(props: DynamicVirtualizedListProps) {
        super(props);
    }

    scrollBy = (scrollOffset: number, scrollBy: number | undefined) => () => {
        ...
        this._updateAtBottom();
        this._scrollCorrectionInProgress = false;
    };

    // Tracks whether the viewport is pinned to the bottom based on the actual
    // DOM scroll position. This is the source of truth for "stay at bottom"
    // decisions; deriving it from totalMeasuredSize is unreliable because
    // measured row heights are ceil-rounded and overestimate the real
    // content height.
    _updateAtBottom = () => {
        const element = this._outerRef;
        if (element) {
            this._listMetaData.atBottom =
                element.clientHeight + element.scrollTop >=
                element.scrollHeight - atBottomMargin;
        }
    };

    scrollTo(scrollOffset: number, scrollByValue?: number, useAnimationFrame = false) {
    ...
    scrollToItem(index: number, align: ScrollAlign = 'auto', offset = 0) {
        ...
        if (!offsetOfItem) {
            const itemSize = getItemMetadata(this.props, index, this._listMetaData).size;
    ...
    getSnapshotBeforeUpdate(
        _: DynamicVirtualizedListProps,
        prevState: DynamicVirtualizedListState,
    ): ScrollSnapshot | null {
    ...
    componentDidUpdate(
        prevProps: DynamicVirtualizedListProps,
        prevState: DynamicVirtualizedListState,
        ...
        if (prevProps.width !== this.props.width) {
            ...
            if (innerRef && typeof innerRef === 'object' && 'current' in innerRef && innerRef.current) {
                this.innerRefWidth = innerRef.current.clientWidth;
            }
            this._widthChange();
        }
    ...
    _callOnScroll = memoizeOne((
        scrollDirection: DynamicVirtualizedListState['scrollDirection'],
        scrollOffset: number,
        ...
        if (typeof this.props.onItemsRendered === 'function') {
            if (itemCount > 0) {
                ...
                if (
                    ...
                    const sizeOfNextElement = getItemMetadata(
                        this.props,
                        overscanStopIndex + 1,
                        this._listMetaData,
                    ).size;
    ...
    _dataChange = () => {
        if (this._listMetaData.totalMeasuredSize < this.props.height) {
            void this.props.canLoadMorePosts();
        }
    };

    _heightChange = () => {
        if (this._listMetaData.atBottom) {
    ...
    _getItemStyle = (index: number): ListItemStyle => {
    ...
    _getRangeToRender(scrollTop?: number): [number, number, number, number] {
        ...
        const scrollOffsetValue = scrollTop !== undefined && scrollTop >= 0 ? scrollTop : scrollOffset;
        ...
        if (
            ...
            const [start, end] = this.props.initRangeToRender;
    ...
    _handleNewMeasurements = (key: ListItemKey, newSize: number, forceScrollCorrection: boolean) => {
        ...
        // atBottom reflects the scroll position from before this row's size
        // changed, so a growing bottom row doesn't unpin the viewport.
        if (
            (this._listMetaData.atBottom || this._keepScrollToBottom) &&
    ...
    _onItemRowUnmount = (itemId: ListItemKey, index: number) => {
        const {props} = this;
    ...
    _outerRefSetter = (ref: HTMLDivElement | null) => {
        ...
        if (typeof outerRef === 'function') {
            ...
            (outerRef as MutableRefObject<HTMLDivElement | null>).current = ref;
        }
    };
...
const getItemOffset = (props: ListMeasurementProps, index: number, listMetaData: ListMetaData) =>
...
const getStartIndexForOffset = (props: ListMeasurementProps, offset: number, listMetaData: ListMetaData) => {
...
const getStopIndexForStartIndex = (
    ...
    while (stopIndex > 0 && offset <= maxOffset) {
        const stopItemMetadata = getItemMetadata(props, stopIndex, listMetaData);
        offset = stopItemMetadata.offset + stopItemMetadata.size;
        stopIndex--;
    }
...
const getItemSize = (props: ListMeasurementProps, index: number, listMetaData: ListMetaData) => {
